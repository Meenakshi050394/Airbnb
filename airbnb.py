...
class features:
    ...
    def feature(column_name, order='count desc', limit=10, constant_for_percentage=55.55):

        try:
            # Connect to MySQL database with context manager
            with mysql.connector.connect(
                ...
                data = pd.DataFrame(cursor.fetchall(), columns=[column_name, 'count'])

                # Custom index formatting function (optional)
                def format_index(x):
                    return '{:^{}}'.format(x, 10)

                data = data.set_axis([format_index(i) for i in data.index], axis=0, inplace=False)

                # Add columns with calculations
                data['S.No'] = data.index + 1  # Alternative for renaming index
                data['percentage'] = data['count'].apply(lambda x: f"{x / constant_for_percentage:.2f}%")
                data['y'] = data[column_name].astype(str)  # Remove backtick if not needed
    ...
    def feature(column_name, order='count desc', limit=10, constant_for_percentage=55.55):
        ...
        try:
            ...
            with mysql.connector.connect(
                ...
                data = pd.DataFrame(results, columns=[column_name, 'count'])

                # Custom index formatting function (optional)
                def format_index(x):
                    return f"S.No. {x}"  # Example custom labeling

                data = data.set_axis([format_index(i) for i in range(1, len(results) + 1)], axis=0)

                # Add columns with calculations
                data['percentage'] = data['count'].apply(lambda x: f"{x / constant_for_percentage:.2f}%")
                data.y = data[column_name]  # Avoid unnecessary conversion if not required
    ...
    def analyze_column(column_name, order='count desc', limit=10, constant_for_percentage=55.55):
        ...
        try:
            ...
            with mysql.connector.connect(
                ...
                cursor.execute(f"""
                    SELECT DISTINCT {column_name}, COUNT({column_name}) AS count
                    FROM airbnb_info
                    WHERE {column_name} != 'Not Specified'  # Filter out 'Not Specified' values
                    GROUP BY {column_name}
                    ORDER BY {order}
                    LIMIT {limit}
                """)
                ...
                # ... (rest of the code for formatting and calculations) ...
    ...
    def get_location_counts():
        ...
        try:
            ...
            with mysql.connector.connect(
                ...
                # Fetch results and create DataFrame
                results = cursor.fetchall()
                data = pd.DataFrame(results, columns=['Country', 'Location Count'])

                # ... (rest of the code for formatting, if needed) ...
    ...
    def feature_analysis():
        ...
        # vertical_bar chart
        property_type = features.feature('property_type')
        ...
        with col1:
            bed_type = features.feature('bed_type')
        ...
        with col2:
            room_type = features.feature('room_type')
        ...
        with tab1:
            minimum_nights = features.feature('minimum_nights')
            plotly.vertical_bar_chart(df=minimum_nights, x='minimum_nights', y='percentage', text='count',
                                      color='#5cb85c', title='Minimum Nights', title_x=0.43)
        with tab2:
            maximum_nights = features.feature('maximum_nights')
            plotly.vertical_bar_chart(df=maximum_nights, x='maximum_nights', y='percentage', text='count',
                                      color='#5cb85c', title='Maximum Nights', title_x=0.43)

        # line chart
        cancellation_policy = features.feature('cancellation_policy')
        ...
        # vertical_bar chart
        accommodates = features.feature('accommodates')
        ...
        with tab1:
            host_response_rate = features.feature('host_response_rate')
        ...
        with tab2:
            host_listings_count = features.feature('host_listings_count')
        ...
        with tab1:
            number_of_reviews = features.feature('number_of_reviews')
        ...
        with tab2:
            max_number_of_reviews = features.feature(
        ...
        with tab3:
            review_scores = features.feature('review_scores')
...
if button and option == 'Migrating to SQL':
    st.write('')
    data_inserting.create_db()
    data_inserting.insert_data()
